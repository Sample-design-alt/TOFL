...
from model.OSCNN.os_cnn_structured_build import generate_layer_parameter_list
...
class Resudual_block(nn.Module):
    def __init__(self, use_res=True):
        super(Resudual_block, self).__init__()
        self.use_res=use_res
        layer_parameter_list1,out = generate_layer_parameter_list(1,
                                                             67,
                                                             [1024, 229376],
                                                             # [8 * 128, 5 * 128 * 256 + 2 * 256 * 128],
                                                             in_channel=1)
        layer_parameter_list2, out= generate_layer_parameter_list(1,
                                                             67,
                                                             [229376, 229376],
                                                             # [8 * 128, 5 * 128 * 256 + 2 * 256 * 128],
                                                             in_channel=out)
        layer_parameter_list3, out = generate_layer_parameter_list(1,
                                                             67,
                                                             [229376, 229376],
                                                             # [8 * 128, 5 * 128 * 256 + 2 * 256 * 128],
                                                             in_channel=out)
        self.block1= OS_CNN_Block(layer_parameter_list1,50)
        self.block2= OS_CNN_Block(layer_parameter_list2,50)
        self.block3= OS_CNN_Block(layer_parameter_list3,50)
    ...
    def forward(self, x):
        ...
        if self.use_res== True:
            # output = self.short_cut(x)+output
            pass
        return output
...
if __name__ == '__main__':
    x = torch.randn(16, 1, 270)

    layer_parameter_list = generate_layer_parameter_list(1,
                                                         67,
                                                         [1024, 229376 ],  #[8 * 128, 5 * 128 * 256 + 2 * 256 * 128],
                                                         in_channel=1)
    # net = OS_CNN(layer_parameter_list, 50)
    net = Resudual_block()
    y=net(x)
